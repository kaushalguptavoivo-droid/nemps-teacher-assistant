...
class _HomeworkScreenState extends ConsumerState<HomeworkScreen>
              ...
              const PopupMenuItem(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text('Import Homework (CSV/Excel)'),
                  ],
                ),
              ),
              const PopupMenuItem(
  ...
  Future<void> _handleImportExport(String action) async {
    if (action == 'import') {
      await _importHomework();
    } else if (action == 'export') {
      await _exportHomework();
    }
  }

  Future<void> _importHomework() async {
    try {
      ...
      final confirmed = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          title: const Text('Import Homework'),
          content: SizedBox(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${data.length - 1} homework items imported!'),
  ...
  Future<void> _exportHomework() async {
    try {
      final data = [
        ['Date', 'Subject', 'Description', 'Class', 'Status']
      ];
      ...
      await Share.shareXFiles(
        [XFile.fromData(bytes, mimeType: 'text/csv', name: 'homework_export.csv')],
        subject: 'Homework Export',
      );

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Homework exported!'), backgroundColor: Colors.green),
        );
      }
    } catch (e) {
      if (mounted) {
