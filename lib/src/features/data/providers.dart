...
final homeworkProvider = FutureProvider.family<List<Homework>, String>((ref, classId) => ref.watch(repoProvider).getHomeworkForClass(classId));
final noticesProvider = FutureProvider.family<List<Notice>, String>((ref, classId) => ref.watch(repoProvider).getNotices(classId));
final themeProvider = StateProvider<ThemeMode>((_) => ThemeMode.system);

final absentStudentsProvider = FutureProvider.family<List<Student>, (String, DateTime)>((ref, params) {
  final (classId, date) = params;
  return ref.watch(repoProvider).getAbsentStudents(classId, date);
});

final dailyAttendanceCountProvider = FutureProvider.family<Map<String, int>, (String, DateTime)>((ref, params) {
  final (classId, date) = params;
  return ref.watch(repoProvider).getDailyAttendanceCount(classId, date);
});

final homeworkStatusProvider = FutureProvider.family<List<HomeworkStatusRecord>, String>((ref, homeworkId) {
