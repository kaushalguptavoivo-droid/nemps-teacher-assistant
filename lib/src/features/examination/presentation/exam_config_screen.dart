...
class _ExamConfigScreenState extends ConsumerState<ExamConfigScreen> {
              ...
              const PopupMenuItem(
                ...
                child: Row(
                  children: [
                    ...
                    SizedBox(width: 8),
                    Text('Import Exam Config (CSV/Excel)'),
                  ],
                ),
              ),
              const PopupMenuItem(
  ...
  Future<void> _handleImportExport(String action) async {
    if (action == 'import') {
      await _importExamConfig();
    } else if (action == 'export') {
      await _exportExamConfig();
    }
  }

  Future<void> _importExamConfig() async {
    try {
      ...
      final confirmed = await showDialog<bool>(
        ...
        builder: (ctx) => AlertDialog(
          title: const Text('Import Exam Configuration'),
          content: SizedBox(
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${data.length - 1} exam configurations imported!'),
  ...
  Future<void> _exportExamConfig() async {
    try {
      final data = [
        ['Class', 'Pattern', 'Passing %', 'Result Type', 'Terms']
      ];
      ...
      await Share.shareXFiles(
        [XFile.fromData(bytes, mimeType: 'text/csv', name: 'exam_config_export.csv')],
        subject: 'Exam Configuration Export',
      );

      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('Exam configuration exported!'), backgroundColor: Colors.green),
        );
      }
    } catch (e) {
      if (mounted) {
