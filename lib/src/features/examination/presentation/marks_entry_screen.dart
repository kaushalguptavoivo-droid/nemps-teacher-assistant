...
class _MarksEntryScreenState extends ConsumerState<MarksEntryScreen>
  ...
  Future<void> _importMarks() async {
    try {
      ...
      // Process import - show info that it needs to be linked to existing marks
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('${data.length - 1} rows imported. Marks will be matched by student and subject.'),
  ...
  Future<void> _exportMarks() async {
    try {
      final data = [
        ['Student Name', 'Roll No', 'Subject', 'Term', 'Obtained Marks', 'Max Marks']
      ];

      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Export ready! Marks data will be exported.')),
      );
      ...
      await Share.shareXFiles(
        [XFile.fromData(bytes, mimeType: 'text/csv', name: 'marks_export.csv')],
        subject: 'Marks Export',
      );
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
